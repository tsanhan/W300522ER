(function (window, document) {
    ...
    const implementColor = () => {
        ...
        document.querySelector('#colorDiv').innerHTML =
            `<div style="width: 50vw; height: 100px; background-color: ${rgb}">${rgb}</div>`;
    }
    ...
    colorizeTextFunc =  () => {
        ...
        innerText
            .split(' ')
            .map(word => word.length > 7 ? `<span style="color: ${rgb}">${word}</span>` : word)
            .join(' ');
